...
QConfig = namedtuple(
    "QConfig",
    "from_dtype, from_scale, from_zero_point, \
...
def quantize(data, output_scale=8.0, output_zero_point=0.0, axis=-1, out_dtype="int8"):
...
def dequantize(data, input_scale, input_zero_point=0.0, axis=-1):
    ...
    return relay.qnn.op.dequantize(data, input_scale, input_zero_point, axis)
...
def add_float(
    ...
    rhs_scale,
    output_scale,
    out_dtype="int32",  # Use int32 to avoid clipping, will be requantized later
):
    """
    Add in float space to avoid int16 overflow.

    This matches PyTorch I-ViT behavior where residual adds happen
    in float space after dequantization.

    Pattern: dequantize(lhs) + dequantize(rhs) -> requantize(result)

    Uses int32 output to avoid clipping. The result will be requantized
    to int8/int16 by the next layer's requantize operation.
    """
    # Dequantize both inputs to float32
    lhs_float = dequantize(lhs, lhs_scale)
    rhs_float = dequantize(rhs, rhs_scale)

    # Add in float space
    result_float = lhs_float + rhs_float

    # Requantize to output scale with int32 to avoid clipping
    result = quantize(result_float, output_scale=output_scale, out_dtype=out_dtype)
...
def quantized_matmul(
    x, y, input_scale1, input_scale2, x_zero_point=0.0, y_zero_point=0.0
):
    ...
    matmul = relay.qnn.op.batch_matmul(
...
def shift_exp(data, input_scale, n):
    data = (
        data
        + relay.right_shift(data, relay.const(1, dtype="int32"))
        - relay.right_shift(data, relay.const(4, dtype="int32"))
    )
...
def quantized_softmax(data, input_scale):
    ...
    # Fix: Use rounding instead of truncation to handle uniform distributions
    # When all exp_int values are equal (uniform attention), each should get 127/N
    # Adding 2^23 (half of 2^24) before right-shifting by 24 implements rounding
    rounding_bias = relay.const(2**23, "int32")
    exp_int = relay.right_shift(
        (factor / exp_int_sum) * exp_int + rounding_bias, relay.const(24, dtype="int32")
    )
...
def quantized_gelu(pre_data, input_scale):
    ...
    sigmoid_int = relay.right_shift(
        (factor / exp_int_sum) * exp_int, relay.const(24, dtype="int32")
    )
